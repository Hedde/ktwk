defmodule IslandsEngine.Game do
  ...
  @spec set_islands(atom | pid | {atom, any} | {:via, atom, any}, :player1 | :player2) :: any
  ...
  defp reply_error(state_data, reply), do: {:reply, reply, state_data, @timeout}
  ...
  def handle_call({:position_island, player, key, row, col}, _from, state_data) do
    ...
    with {:ok, rules} <- Rules.check(state_data.rules, {:position_islands, player}),
      ...
      {:error, :overlapping_island} ->
        reply_error(state_data, {:error, :overlapping_island})
